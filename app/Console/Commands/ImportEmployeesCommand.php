...
class ImportEmployeesCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Import employees and profiles from a .xlsx file';
    ...
    public function handle()
    {
        ...
        if (!$headerRow) {
            $this->error("Excel file is empty or missing headers.");
            return 1;
        }
        ...
        foreach ($essentialHeaders as $header) {
            if (!isset($headersMap[$header])) {
                $this->error("Missing essential header: '{$header}'");
                return 1;
            }
        }
        ...
        try {
            ...
            DB::rollBack();
            $this->error("A catastrophic error occurred. Transaction rolled back.");
        ...
        $this->table(
            ...
            [
                ...
                ['Users Created', $usersCreatedCount],
        ...
        if (count($errors) > 0) {
            ...
            $this->table(
                ['Row', 'Reason'],
                array_map(fn($e) => [$e['row'], $e['reason']], $errors)
            );
        }
